"""Checking an exact Sabueso Conda release route and its public outcome."""
...
# Sabueso runs its whole supported matrix (Python 3.11-3.14) on every push.
FULL_MATRIX = ".github/workflows/ci.yml"
...
def _read_json(url: str, *, token: str | None = None) -> dict:
    headers = {"Accept": "application/json", "User-Agent": "sabueso-release-route"}
...
def assert_version_unoccupied(version: str) -> dict:
    ...
    url = f"https://api.anaconda.org/release/uibcdf/sabueso/{quote(version, safe='')}"
    try:
    ...
    raise ReleaseRouteError(
        f"Sabueso {version} already has {len(distributions)} distribution records; "
...
def check_route(
    *, version: str, route: str, sha: str, repository: str, receipt: Path
    ...
    if repository != "uibcdf/sabueso":
        raise ReleaseRouteError("release route must run in uibcdf/sabueso")
    plan = read_plan()
...
def verify_public(
    *, version: str, built_paths: str, receipt: Path, attempts: int = 6
    ...
    if len(paths) != 1:
        raise ReleaseRouteError("Sabueso noarch release must build exactly one file")
    package = Path(paths[0])
    expected_name = f"sabueso-{version}-py_0.tar.bz2"
    ...
    for attempt in range(attempts):
        ...
        if result.returncode == 0:
            records = json.loads(result.stdout).get("sabueso", [])
            matching = [
                record
            ...
            if len(matching) == 1:
                evidence["public"] = {
                    "coordinate": f"uibcdf/sabueso/{version}/noarch/{expected_name}",
                    "sha256": digest,
