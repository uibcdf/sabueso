...
def test_end_to_end_protein_with_multiple_sources_offline():
    # Legacy unguarded merge (no entity_subjects): the InterPro entry is a family, not the
    # protein. Kept until annotation sources become typed relationships (#21); structures
    # are relationships now, see test_protein_cards_offline.py.
    uniprot = _load("temp_data/P52789.json")

    interpro_path = Path("temp_data/IPR000001.json")
    if not interpro_path.exists():
        pytest.skip("Missing temp_data for InterPro offline test.")
    ...
    merged = merge_mapping_results([uni_map, ipr_map])
