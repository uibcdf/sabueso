...
from sabueso.tools.db.go import create_go_card_from_json
...
def _cards():
    ...
    go_term = create_go_card_from_json(
        {"id": "GO:0004807", "label": "triose-phosphate isomerase activity"},
        retrieved_at="2026-02-01",
    )
    return hk2, hs_tim, tc_tim, go_term


def test_sort_uses_resolved_values_and_puts_missing_last():
    hk2, hs_tim, tc_tim, go_term = _cards()
    deck = Deck([hk2, go_term, tc_tim, hs_tim])

    ascending = deck.sort("sequence.length")
    assert [c.id for c in ascending.cards] == [hs_tim.id, tc_tim.id, hk2.id, go_term.id]
