...
def test_pubchem_pc_compounds_record_offline():
    ...
    card = create_compound_card_from_file("temp_data/66414.json", retrieved_at="2026-02-04")
    ...
    assert card.get("identifiers.smiles")["value"] == "CC12CCC3C(C1CCC2O)CCC4=CC(=C(C=C34)OC)O"
    for fp in ("properties.physchem.logp", "properties.physchem.tpsa", "properties.physchem.hbd",
               "properties.physchem.hba", "properties.physchem.rotatable_bonds", "identifiers.inchi"):
