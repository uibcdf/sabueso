"""PDBe-KB ligand binding sites of a protein (uibcdf/sabueso#28).
...
class OnlinePDBeKBClient:
    ...
    def ligand_sites(self, accession: str) -> Dict[str, Any]:
        url = f"{PDBE_GRAPH_API}/uniprot/ligand_sites/{accession}"
        ...
        try:
            ...
            if exc.code == 404:
                raise RecordNotFoundError(
                    f"PDBe-KB has no ligand sites for {accession}"
                ) from exc
        ...
        if not record:
            raise RecordNotFoundError(f"PDBe-KB has no ligand sites for {accession}")
...
class FixturePDBeKBClient:
    ...
    def ligand_sites(self, accession: str) -> Dict[str, Any]:
        ...
        path = self.directory / "pdbe_kb" / f"ligand_sites__{accession}.json"
        if not path.is_file():
            raise RecordNotFoundError(f"PDBe-KB has no ligand sites for {accession}")
