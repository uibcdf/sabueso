...
def fetch_string_network(identifiers: str, species: int = 9606) -> List[Dict[str, Any]]:
    ...
    url = (
        f"https://string-db.org/api/json/network?identifiers={ident}&species={species}"
    )
    with urlopen(url) as resp:  # nosec - expected trusted endpoint
        return json.loads(resp.read().decode("utf-8"))


def create_string_card_online(
    identifiers: str, retrieved_at: str | None = None, species: int = 9606
) -> Any:
    ...
    mapping = map_string_interactions(
        data, query_name=identifiers, retrieved_at=retrieved_at or _now_date()
    )
