...
def map_protein(uniprot_json: Dict[str, Any], retrieved_at: str) -> Dict[str, Any]:
    ...
    if name:
        ...
        assertion = make_source_assertion(
            fp, name, "UniProt", primary or "", retrieved_at
        )
        source_assertions.append(assertion)
        field_source_assertions[fp] = [assertion["id"]]
    ...
    if func_texts:
        ...
        for txt in func_texts:
            assertion = make_source_assertion(
                fp, txt, "UniProt", primary or "", retrieved_at
            )
            source_assertions.append(assertion)
    ...
    if org_name:
        ...
        fields[fp] = org_name
        assertion = make_source_assertion(
            fp, org_name, "UniProt", primary or "", retrieved_at
        )
        source_assertions.append(assertion)
        field_source_assertions[fp] = [assertion["id"]]
    ...
    if (
        ...
        if binding_items:
            ...
            for item in binding_items:
                assertion = make_source_assertion(
                    fp, item, "UniProt", primary or "", retrieved_at
                )
                source_assertions.append(assertion)
                sa_ids.append(assertion["id"])
            field_source_assertions[fp] = sa_ids
